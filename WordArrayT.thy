...
abbreviation "w_len h w \<equiv> len_C (h w)"
...
lemma abs_length_eq:
  "\<alpha> s xs w \<Longrightarrow> unat (w_len (heap_WordArray_u32_C s) w) = length xs"
...
lemma fold_loop_inv:
  "\<lbrace>\<lambda>s. \<alpha> s xs w \<and> r = (xa, y) \<and> y < ret \<and> ret = min to (w_l s w) \<and> frm \<le> y \<and>
   ...
   \<lbrace>\<lambda>xe a. \<alpha> a xs w \<and> ret = min to (len_C (heap_WordArray_u32_C a w)) \<and> frm \<le> y + 1 \<and>
           w_fold xs (unat frm) (unat to) (f_n f_num) acc obsv = 
...
lemma wordarray_fold_refinement:
  "\<lbrace>\<lambda>s. \<alpha> s xs w \<rbrace>
   wordarray_fold_no_break_0' (t5_C acc w f_num frm obsv to)
...
lemma map_loop:
  "\<lbrace>\<lambda>s.  b = y \<and> y < ret \<and> ret = min to (w_l s w) \<and> (frm \<le> ret \<longrightarrow> y \<le> ret) \<and> 
   ...
   \<lbrace>\<lambda>xe s.
    ...
    ret = min to (len_C (heap_WordArray_u32_C s w)) \<and>
...
lemma wordarray_map_refinement:
  "\<lbrace>\<lambda>s. \<alpha> s xs w \<rbrace>
   wordarray_map_no_break_0' (t9_C acc w n_num frm obsv to)
  ...
  apply wp
       ...
       apply (case_tac "min to (len_C (heap_WordArray_u32_C s w)) < frm")
        apply (case_tac "b > to")
       ...
       apply clarsimp
       apply (subgoal_tac "frm \<le> to \<and> frm \<le> len_C (heap_WordArray_u32_C s w)")
        apply clarsimp
        apply (case_tac "b = to")
         apply simp
